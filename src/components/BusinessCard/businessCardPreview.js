...
function BusinessCardPreview({ selectTheme, values }) {
  ...
  return (
    <div className="flex flex-col justify-center items-center h-0 md:h-screen mt-[40rem] md:mt-0">
      {loading ? (
        <Loader />
      ) : (
          <div className="flex flex-col justify-center items-center">
            <div className="md:rounded-3xl w-full md:w-80 h-full relative scrollable-container overflow-y-auto overflow-x-hidden md:h-30 bg-white shadow-xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className={`${themeClass} h-80 md:h-72 md:rounded-t-3xl`}>
                ...
                {(values?.email || cardData?.email) && (
                  <>
                    ...
                    <Link
                      ...
                      onClick={() =>
                        handleMail(values?.email || cardData?.email)
                      }
                    >
                      {values?.email || cardData?.email}
                    </Link>
                ...
                {(values?.website || cardData?.website) &&
                  cardData?.website !== "undefined" && (
                    <>
                      ...
                      <Link
                        className="text-blue-800 cursor-pointer pb-4"
                        onClick={() =>
